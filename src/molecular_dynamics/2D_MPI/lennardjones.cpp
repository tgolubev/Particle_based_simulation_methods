   ...
   //First calculate interactions btw. all atoms in the processor's domain
    for(int current_index = 0; current_index < system.num_atoms(); current_index++) {
        ...
        for(int other_index = current_index+1; other_index < system.num_atoms(); other_index++) {   //to avoid double counting, other_index = current_index+1
            ...
            //NOTE: For 1D decomposition, need minimum image convention along y-direction!--> b/c no passing of atoms/ghosts etc there!
            ...
            //Note: attractive force points towards other atom.
            for(int j=0;j<2;j++) {
